...
- **Event envelope drift.** Orders' event DTOs (`orders/.../event/dtos/EventDtos.kt`) diverge from the frozen contract in `api-design.md`: the version field is `payloadVersion`, not `schemaVersion`; fields are flat with no `payload {}` wrapper; `occurredAt` is a `LocalDateTime` (no timezone offset), not an ISO instant. The consumer deliberately mirrors the **as-built** shape (with `@JsonIgnoreProperties(ignoreUnknown = true)` for forward-compat) rather than the aspirational doc. The team should decide whether to reconcile the producer to the doc or update the doc to match; the consumer follows whichever wins.
